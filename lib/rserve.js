...
function createFacade(state) {
    return {
        ...
        evaluate : function(command, cb) {
            if (state.closed) {
                state = state.reconnect(function (err) {
                    if (err) {
                        return cb(err);
                    }
                    evaluate(state.client, cb, command);
                });
            } else {
                evaluate(state.client, cb, command);
            }
...
function connect(host, port, cb) {
    var state = {
        ...
        reconnect: function (cb) {
            return connect(host, port, cb);
        }
    };

    state.client.buffers = buffers();
    state.client.handler = function(err) {
        var rv;
        if(err) {
            state.client.handler = noop;
            cb(err, null);
            return;
        }
        if(state.client.buffers.length >= 32) {
            state.client.handler = noop;
            rv = state.client.buffers.splice(0, 32).slice(4, 8).toString('utf8');
            if (rv !== '0103') {
                cb(new Error('Unsupported protocol version ' + rv), null);
            } else {
                cb(null, createFacade(state));
            }
        }
    };
    state.client.on('error',function(err){
        state.client.handler(err);
    });
    state.client.on('end', function () {
        state.closed = true;
        state.client.handler(new Error('Connection to R ended unexpectedly'));
    });
    state.client.on('close', function () {
        state.closed = true;
        state.client.handler(new Error('Connection to R closed unexpectedly'));
    });
    state.client.on('data', function(data) {
        state.client.buffers.push(data);
